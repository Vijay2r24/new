...
// import 'react-phone-input-2/lib/style.css' // Import the styles for react-phone-number-input
...
const Contact = () => {
  const form = useRef();
  const [phoneNumber, setPhoneNumber] = useState("");
  const [countryCode, setCountryCode] = useState("in");  // Corrected to lowercase 'in' for PhoneInput compatibility
  const [phoneInputDisabled, setPhoneInputDisabled] = useState(false);
  ...
  const handlePhoneInputChange = (value) => {
    ...
    if (value !== undefined && value !== null) {
      // Limit phone number length based on country code
      const maxLength = getMaxLengthForCountry(countryCode.toUpperCase());
      if (value.length > maxLength) {
        setPhoneInputDisabled(true); // Disable input field if max length exceeded
      } else {
        setPhoneInputDisabled(false);
  ...
  const sendEmail = (e) => {
    ...
    e.target.reset(); // Reset form after submission
    setPhoneNumber(""); // Reset phone number state
    setCountryCode("in"); // Reset country code state to default "IN"
    setPhoneInputDisabled(false); // Reset phone input disabled state
  };
        ...
        <div className="contact-us-right">
          <form ref={form} onSubmit={sendEmail}>
            ...
            <PhoneInput
              ...
              value={phoneNumber}
              onChange={handlePhoneInputChange}
              country={countryCode} // Updated to use the `country` prop
              name="user_phone"
              disabled={phoneInputDisabled}
            />
            <textarea
